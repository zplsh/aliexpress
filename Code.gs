...
function getByPlatformAndroidSheet(){
  var ss = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('conversions Android')
  var lr = ss.getLastRow()
  var lc = ss.getRange(1, 1).getDataRegion().getLastColumn()
  ss.getRange(2,1, lr, lc).clearContent()
  return ss
...
function getByPlatformIOSSheet(){
  var ss = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('conversions IOS')
  var lr = ss.getLastRow()
  var lc = ss.getRange(1, 1).getDataRegion().getLastColumn()
  ss.getRange(2,1, lr, lc).clearContent()
  return ss
...
query += ', CASE\n'
query += 'WHEN LOWER(goal) = \'new_buyer_purchase\'\n'
query += 'THEN conversions\n'
...
query += '1048, 1046)\n'
query += 'AND lower(affiliate_info1) ~ ' + '\'' +'\\d{5,10}' + '\'' + ') AS foo\n'
...
query += 'JOIN rawdb.mytarget.banner b\n'
query += 'ON bs.banner_id = b.banner_id\n'
...
query += ', h.campaign_id\n'
query += ', m.campaign_name\n'
query += ', h.date\n'
query += ', m.impressions                                            mt_imps\n'
query += ', m.clicks                                              AS mt_clicks\n'
query += ', ROUND(m.clicks::numeric / m.impressions::numeric, 3)  AS CTR\n'
query += ', h.installs\n'
query += ', h.purchases\n'
query += ', CASE\n'
query += 'WHEN h.installs = 0\n'
query += 'THEN 0\n'
query += 'ELSE ROUND(h.purchases::numeric / h.installs::numeric, 3) END AS CR_new_buyer_purchase\n'
query += ', m.costs\n'
query += ', h.purchases * 21                                      AS revenue_usd\n'
query += ', h.purchases * 21 - m.costs                            AS profit\n'
query += ', CASE\n'
query += 'WHEN h.purchases = 0\n'
query += 'THEN 0\n'
query += 'ELSE ROUND(m.costs::numeric / h.purchases, 3) END AS CPA\n'
query += ', CASE\n'
query += 'WHEN h.installs = 0\n'
query += 'THEN 0\n'
query += 'ELSE ROUND(m.costs::numeric / h.installs, 3) END  AS CPI\n'
query += 'FROM h\n'
query += 'JOIN m ON h.campaign_id = m.campaign_id\n'
query += 'AND h.date = m.date\n'
query += 'WHERE h.date BETWEEN ' + '\'' + dates[0] + '\'' + 'AND' + '\'' + dates[1] + '\'' +'\n'
query += 'AND m.date BETWEEN ' + '\'' + dates[0] + '\'' + 'AND' + '\'' + dates[1] + '\'' +'\n'
query += 'ORDER BY m.clicks DESC;'
...
function mtAndroidReportAddQuery(){
var dates = getDates()
var query = ''
query += 'WITH h AS (\n'
query += 'SELECT campaign_id\n'
...
query += 'JOIN rawdb.mytarget.campaign c ON b.campaign_id = c.campaign_id\n'
query += 'WHERE 1 = 1\n'
query += 'AND lower(c.name) ~ \'android|andr|and\'\n' //android/ios 
query += 'GROUP BY c.campaign_id\n'
query += ', c.name\n'
query += ', date)\n'
query += 'SELECT CASE\n'
...
query += '           ELSE \'Android\' END                                                      AS platform\n'
query += '     , h.date\n'
query += '     , SUM(m.impressions)                                                             mt_imps\n'
query += '     , SUM(m.clicks)                                                               AS mt_clicks\n'
query += '     , CASE\n'
query += '           WHEN SUM(m.impressions) = 0\n'
query += '               THEN 0\n'
query += '           ELSE ROUND(SUM(m.clicks::numeric) / SUM(m.impressions::numeric), 3) END AS CTR\n'
query += '     , SUM(h.installs)                                                             AS installs\n'
query += '     , SUM(h.purchases)                                                            AS purchases\n'
query += '     ,\n'
query += '    CASE\n'
query += '        WHEN SUM(h.installs) = 0\n'
query += '            THEN 0\n'
query += '        ELSE\n'
query += '            ROUND(SUM(h.purchases::numeric) / SUM(h.installs::numeric), 3) END     AS CR_new_buyer_purchase\n'
query += '     , SUM(m.costs)                                                                AS costs\n'
query += '     , SUM(h.purchases) * 21                                                       AS revenue_usd\n'
query += '     , SUM(h.purchases) * 21 - SUM(m.costs)                                        AS profit\n'
query += '     , CASE\n'
query += '           WHEN SUM(h.purchases) = 0\n'
query += '               THEN 0\n'
query += '           ELSE\n'
query += '               ROUND(SUM(m.costs::numeric) / SUM(h.purchases), 3) END              AS CPA\n'
query += '     , CASE\n'
query += '           WHEN SUM(h.installs) = 0\n'
query += '               THEN 0\n'
query += '           ELSE ROUND(SUM(m.costs::numeric) / SUM(h.installs), 3) END              AS CPI\n'
query += 'FROM h\n'
query += '         JOIN m ON h.campaign_id = m.campaign_id\n'
query += '    AND h.date = m.date\n'
query += 'WHERE h.date BETWEEN ' + '\'' + dates[0] + '\'' + 'AND' + '\'' + dates[1] + '\'' +'\n'
query += 'AND m.date BETWEEN ' + '\'' + dates[0] + '\'' + 'AND' + '\'' + dates[1] + '\'' +'\n'
query += 'GROUP BY platform, h.date\n'
query += 'ORDER BY SUM(m.clicks) DESC;'
...
var url = "http://diet-secrets.club/api/as_apis/v01/?token=STAryPHaRIFoREfredro&db=rawdb";
  ...
  var a = JSON.parse(result.getContentText());
  Logger.log(a)
  var targetSheet = getByPlatformAndroidSheet()
  setRowsDataStat(targetSheet, a, 'android')

}

function mtIOSReportAddQuery(){
var dates = getDates()
var query = ''
query += 'WITH h AS (\n'
query += 'SELECT campaign_id\n'
...
var url = "http://diet-secrets.club/api/as_apis/v01/?token=STAryPHaRIFoREfredro&db=rawdb";
  ...
  var a = JSON.parse(result.getContentText());
  Logger.log(a)
  var targetSheet = getByPlatformIOSSheet()
  setRowsDataStat(targetSheet, a, 'ios')
}
